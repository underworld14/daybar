...
def emit_web_sheet(imgs, scale=4, sheet_w=256, pad=1):
    ...
    meta = {"scale": scale, "tile": 32, "sheet": [sheet_w, sheet_h], "frames": frames}
    (WEB / "garden-frames.json").write_text(json.dumps(meta, indent=2, sort_keys=True) + "\n")
    # Also emit a JS global so the site works without fetch() (GitHub Pages AND local file://).
    (WEB / "garden-frames.js").write_text(
        "// Generated by Scripts/generate_garden_pixels.py — do not edit.\n"
        "window.GARDEN_FRAMES = " + json.dumps(meta, sort_keys=True) + ";\n"
    )
    print(f"  wrote garden-sheet.png ({sheet_w*scale}x{sheet_h*scale}) + garden-frames.json/.js -> {WEB.relative_to(ROOT)}")
